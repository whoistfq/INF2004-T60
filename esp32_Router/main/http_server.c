...
static esp_err_t index_get_handler(httpd_req_t *req)
{
    ...
    // Redirect to Captive Portal on startup
    httpd_resp_sendstr(req, "HTTP/1.1 302 Found\r\nLocation: /captive_portal\r\n\r\n");

    return ESP_OK;
}

/* Captive Portal route handler */
static esp_err_t captive_portal_get_handler(httpd_req_t *req)
{
    const char* resp_str = CAPTIVE_PORTAL_PAGE;
    httpd_resp_send(req, resp_str, strlen(resp_str));
    return ESP_OK;
}

/* Agree button handler */
static esp_err_t agree_get_handler(httpd_req_t *req)
{
    // Add any logic you need when the user agrees (e.g., set a flag, redirect to the main configuration page)
    httpd_resp_send(req, "Agreed! You can now access the network.", HTTPD_RESP_USE_STRLEN);
    return ESP_OK;
}
...
httpd_uri_t agree_uri = {
    .uri = "/agree",
    .method = HTTP_GET,
    .handler = agree_get_handler,
    .user_ctx = NULL};
...
httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(server, &agree_uri);
